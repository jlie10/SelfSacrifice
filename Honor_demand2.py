...
########################################
    ...
    def genemap(self):
        ...
        """
        return [('Patriot_Inv'), ('NonPatriot_Inv'), ('Patriot_Req'), ('NonPatriot_Req')] 		# gene length (in bits) is read from configuration
    ...
    def honoring(self, worshippers, nb_heroes):        
        ...
        for Indiv in worshippers:
            ...
            if Indiv.Patriotism == 0:
                offering = Indiv.gene_relative_value('NonPatriot_Inv')
                ...
                offering = Indiv.gene_relative_value('Patriot_Inv')
                Indiv.score (-self.costHonor(offering))
...
########################################
    ...
    def interact(self, indiv, Signalers): # NOT USED in inter2
        ...
        if indiv.Patriotism == 0:
            demand = indiv.gene_relative_value('NonPatriot_Req')
        else:
            demand = indiv.gene_relative_value('Patriot_Req')
        ...
        for Signaler in Signalers:
            ...
            if Signaler.Patriotism == 0 and offer < Signaler.gene_relative_value('NonPatriot_Req'):
                continue
            if Signaler.Patriotism == 1 and offer < Signaler.gene_relative_value('Patriot_Req'):
                continue
...
class Patriotic_Individual(Base.Individual, EA.Friend):
