...
class Individual(EI.Individual):
	...
	def locate(self, NewPosition, Erase=True):
		...
		Observer.record((self.ID, self.location + (self.Colour, -1))) # for ongoing display
		return True
	...
	def erase(self):
		...
		if self.location:
			...
			Observer.record((self.ID, self.location + (NColour, -1)))
...
if __name__ == "__main__":
	...
	Observer.recordInfo('DefaultViews',	[('Field', 400, 340)])	# Evolife should start with these windows open - these sizes are in pixels
