...
class Agent:
	...
	def __init__(self, IdNb):
		...
		if IdNb % 2:	self.colour = EC.Shade(IdNb, BaseColour='red', Max=100, darkToLight=False)
		else:			self.colour = EC.Shade(IdNb, BaseColour='blue', Max=100)
		self.Size = 5
...
class Population:
	...
	def one_year(self):
		...
		self.Obs.record(list(self.positions().items()), Window='Field', Reset=True)	# let Observer know position changes
		...
		MC = self.Obs.get_info('MouseClick')
		if MC and MC != self.MouseEvent:
			self.MouseEvent = MC
			print(MC)
