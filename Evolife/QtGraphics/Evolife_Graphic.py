...
class AreaView(QtWidgets.QGraphicsView):
	...
	def __init__(self, AreaType=Image_Area, parent=None, image=None, width=400, height=300):
		...
		if AreaType:
			self.Area = AreaType(image, width=width, height=height, EventInterpreter=self.EventInterpreter)	 # View is a kind of camera on Area
...
class Satellite_window(Active_Frame):
	...
	def dimension(self, *geometry):
		if len(geometry) == 1:	
			...
			geometry.reverse()
			geometry += [None, None][:4-len(geometry)]
	...
	def Field_display(self, Layout=None, Photo=0, CurrentFrame=-1, Ongoing=False, Prefix=''):
		...
		if AgentLayout:
			...
			if not Ongoing:	self.Area.remove_absent(AgentLayout.keys())
				# for agent in self.Area.on_ground():
