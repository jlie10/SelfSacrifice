...
class Dialect:
	...
	def sniff(self, fileName, verbose=True):
		...
		elif verbose:	trace(1, 'ERROR:	Unable to open %s for sniffing' % fileName)
		return D
...
def reader(csvFileName, dialect=None, CryptKey=None, verbose=True):
	...
	for Line in openFile(csvFileName, CryptKey=CryptKey, verbose=verbose):
		# try:	print(Line)
...
def load(csvFileName, dialect=None, sniff=False, verbose=True):
	" Loads data from a csv file "
	if dialect is None:	dialect = Dialect()
	if sniff:	dialect = dialect.sniff(csvFileName, verbose=False)
	return reader(csvFileName, dialect, verbose=verbose)
	# return [R for R in Reader]
	
def loadTable(csvFileName, dialect=None, sniff=True, verbose=True):
	" Loads table from a csv file (first line expected to be header) and returns list of dicts "
	T = list(load(csvFileName, dialect=dialect, sniff=sniff, verbose=verbose))
	if T:
